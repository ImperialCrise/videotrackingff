...
def load_detections(det_file):
    """
    Load detections from MOT-challenge formatted file.
    Format: frame, id, bb_left, bb_top, bb_width, bb_height, conf, x, y, z
    
    Returns:
        Dictionary mapping frame_id to list of detections [x, y, w, h, conf]
    """
...
def generate_colors(n_colors):
    """
    Generate n distinct colors for track visualization.
    """
    np.random.seed(42)
    colors = []
...
def main():
    ...
    for img_file in img_files:
        ...
        info_text = f"Frame: {frame_id} | Tracks: {len(active_tracks)}"

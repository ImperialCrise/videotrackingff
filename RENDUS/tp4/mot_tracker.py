...
from kalman_iou_reid_tracker import KalmanIOUReIDTracker
...
def main():
    ...
    for idx, img_file in enumerate(img_files):
        ...
        active_tracks = tracker.update(frame, det_bboxes, frame_id)
        ...
        info_text = f"Frame: {frame_id} | Tracks: {len(active_tracks)} | Kalman+IoU+ReID"
        cv2.putText(frame, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        
        cv2.putText(frame, "Blue: Prediction | Color: Estimated (ReID)", (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        video_out.write(frame)
        
        cv2.imshow('Kalman+IoU+ReID MOT Tracking', frame)
